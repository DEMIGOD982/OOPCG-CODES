/*Write C++ program using STL for Sorting and searching with 
...
public:
 ...
 void get();
 void display();
 bool operator < (const record& rhs) //if you dont write 
this function sort() will compare memory byte by byte
 { 
 ...
 { 
 if(r1.name == r2.name || r1.dob == r2.dob || 
r1.phone == r2.phone) 
 { 
 return true; 
 } 
...
{ 
 for(vector<record>::iterator itr=myRecords.begin();itr
!= myRecords.end();itr++) 
 { 
 itr->display();
 } 
...
{ 
 ...
 rhs.dob = data; 
 rhs.phone = data; 
 vector<record>::iterator itr = find(myRecords.begin(), 
myRecords.end(), rhs);
...
entry\n4. Sort records\nEnter choice\n"; 
 ...
 break; 
 case 3: 
 cout<<"Enter either name, d.o.b or phone number 
you want to find\n"; 
 cin>>key; 
 obj.search(key);
 break; 
